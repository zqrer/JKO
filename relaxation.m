function [c m status] = relaxation(c, m, N, dx, dt, D, z, dtbeta, V, j)
dt1 = dt/10;
flag1 = 0;
j1 = 1;
while j1 <= 10
   [u, info] = opt_func(c, m, N, dx, dt1, D, z, dtbeta, V, flag1);
  if info.status < 0 
     ...
     elseif info.status == -2 && flag1 == 1
        ...
        flag2 = 0;
        while j2 <= 10
           [u info] = opt_func(c, m, N, dx, dt2, D, z, dtbeta, V, flag2);
           if info.status < 0
