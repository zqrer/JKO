function [c m status] = relaxation2(c, m, N, Dx, Dx_mid, dt, D, z, dtbeta, V, j, g)
dt1 = dt/10;
flag1 = 0;
j1 = 1;
while j1 <= 10
   [u, info] = opt_func2(c, m, N, Dx, Dx_mid, dt1, D, z, dtbeta, V, flag1, g);
  if info.status < 0 
     ...
     elseif info.status == -2 && flag1 == 1
        ...
        flag2 = 0;
        while j2 <= 10
           [u info] = opt_func2(c, m, N, Dx, Dx_mid, dt2, D, z, dtbeta, V, flag2, g);
           if info.status < 0
